...
namespace Vella.Events
{
    ...
    public unsafe struct StructuralChangeProxy
    {
        ...
        public static void Initialize()
        {
            ...
            var type = FindType("Unity.Entities.StructuralChange");

            //var member = type.GetMember("CreateEntity", BindingFlags.Public | BindingFlags.Static);

            FieldInfo info = type.GetField("CreateEntity", BindingFlags.Public | BindingFlags.Static);
            
            var f2 = info.GetType().GetMethod("Invoke");

            object yourField = info.GetValue(null);

            MethodInfo method = yourField.GetType().GetMethod("Invoke");

            var d = CreateCompatibleDelegate<CreateEntityDelegate>(yourField, method);
            ...
            //var method2 = type.GetMethod("CreateEntity", BindingFlags.NonPublic | BindingFlags.Static);

            //var ptr = Marshal.GetFunctionPointerForDelegate(f2);

            //CreateEntity = new FunctionPointer<CreateEntityDelegate>(ptr).Invoke;

            //BurstCompiler.CompileFunctionPointer<CreateEntityDelegate>(CreateEntityExecute).Invoke;

            //Delegate.CreateDelegate(getType(types.ToArray()), methodInfo);

            //CreateEntity = (CreateEntityDelegate)CreateDelegate(method, CreateEntity);

            //CreateEntity = typeof() CreateDelegate()

            //(T)(object)Marshal.GetDelegateForFunctionPointer(_ptr, typeof(T));
